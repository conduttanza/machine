...
class Window():
    ...
    def main(self):
        try:
            ...
            slider_servo_1 = pygame.Rect(
                    config.slider_x,
                    config.slider_servo_y,
                    config.slider_len,
                    config.slider_height)
            slider_servo_2 = pygame.Rect(
                    config.slider_x,
                    config.slider_servo_y+config.slider_height+margin,
                    config.slider_len,
                    config.slider_height
                )
            #DEFCON 1
            ...
            while self.running: 
                ...
                pygame.draw.rect(self.screen, (config.BLUE), slider_servo_2)
                
                Label.show_labels()
                #GET THE POSITION FOR THE SLIDER MARKER
                self.mouse_pos = pygame.mouse.get_pos()
                move_motor_slider = getattr(self, 'move_motor_slider', False)
                move_servo_1_slider = getattr(self, 'move_servo_1_slider', False)
                move_servo_2_slider = getattr(self, 'move_servo_2_slider', False)
                ...
                if move_motor_slider and config.slider_x <= self.mouse_pos[0] <= config.slider_x+config.slider_len:
                    ...
                    self.speed = (
                        self.mouse_pos[0] - (config.slider_x+config.slider_len/2)
                        )/2
                    self.lastMotorPos = self.mouse_pos
                    #draw the last known other sliders
                    last_servo_1 = getattr(self,'lastServoPos_1',(300,0))
                    last_servo_2 = getattr(self,'lastServoPos_2',(300,0))
                    pygame.draw.circle(
                            self.screen,
                            (config.BLACK),
                            (last_servo_1[0],config.slider_servo_y+config.slider_height/2),
                            10
                        )
                    pygame.draw.circle(
                            self.screen,
                ...
                elif move_servo_1_slider and config.slider_x <= self.mouse_pos[0] <= config.slider_x+config.slider_len:
                    #draw this slider updating
                    ...
                    pygame.draw.circle(self.screen,(config.BLACK),(last_servo_1[0],config.slider_servo_y+config.slider_height/2),10)
                #MOTOR
                ...
                #SERVO 1    
                    if getattr(self, 'lastServoPos_1', None) != None:
                        pygame.draw.circle(self.screen,(config.BLACK),(self.lastServoPos_1[0],config.slider_servo_y+config.slider_height/2),10)
                ...
                #SERVO 2
                    if getattr(self, 'lastServoPos_2', None) != None:
                        pygame.draw.circle(self.screen,(config.BLACK),(self.lastServoPos_2[0],config.slider_servo_y+config.slider_height+margin+config.slider_height/2),10)
                        self.move_servo_2_slider = False
                    else:#IF SHOULDNT MOVE SERVO 2 SLIDER
                        pygame.draw.circle(
                                self.screen,
                                (config.BLACK),
                                (300,config.slider_servo_y+config.slider_height+margin+config.slider_height/2),
                                10
                            )
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if slider_servo_2.collidepoint(event.pos):
                            self.move = True
